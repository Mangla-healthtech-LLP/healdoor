...
import { getProducts, getServices, getMediaUrl, getMediaAlt } from '@healdoor/utils'
...
export default async function SearchPage({ searchParams }: Props) {
  ...
  const filteredServices = q ? services.filter(s => s.name.toLowerCase().includes(q) || (s.description ? JSON.stringify(s.description).toLowerCase().includes(q) : false)) : [];
    ...
    <>
      ...
      <main className="flex-1 min-h-screen bg-section-alt-bg">
        <PageHeader 
          ...
          description={q ? `Showing results for "${q}"` : "Enter a search query to find products and services"}
          breadcrumbs={[
            { label: 'Home', href: '/' },
        ...
        <section className="section-padding-sm container py-12">
          ...
          <div className="mb-8 md:hidden">
            <form action="/search" method="GET" className="relative w-full">
              ...
              />
              <button type="submit" className="absolute right-1.5 top-1.5 h-9 w-9 rounded-full bg-teal text-white flex items-center justify-center cursor-pointer hover:bg-teal-dark transition-colors">
          ...
          {!q ? (
            <div className="text-center py-20 bg-white rounded-2xl border border-border/30">
              ...
              <p className="text-text-muted text-lg">Please enter a search term above to find what you&apos;re looking for.</p>
            </div>
          ) : (
            <div className="space-y-12">
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-text-dark">Services ({filteredServices.length})</h2>
                  {filteredServices.length > 0 && (
                    <Link href="/services" className="text-sm font-semibold text-teal hover:underline">
                      View all services
                    </Link>
                  )}
                </div>
                ...
                {filteredServices.length === 0 ? (
                  <div className="bg-white p-8 rounded-xl border border-border/50 text-center">
                    <p className="text-text-muted">No services found for &quot;{q}&quot;</p>
                  </div>
                ) : (
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-text-dark">Products ({filteredProducts.length})</h2>
                ...
                {filteredProducts.length === 0 ? (
                  <div className="bg-white p-8 rounded-xl border border-border/50 text-center">
                    <p className="text-text-muted">No products found for &quot;{q}&quot;</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filteredProducts.map((product, i) => {
                      const imageUrl = getMediaUrl(product.image as Parameters<typeof getMediaUrl>[0]) || "/images/service-medical.png";
                      return (
                        <Link
                          key={product.id || i}
                            ...
                            <div className="flex items-baseline gap-1 pt-2 border-t border-border/30">
                              {product.isAvailableForPurchase ? (
                                <span className="text-lg font-bold text-teal">
                                  ₹{(product.buyPrice || 0).toLocaleString("en-IN")}
                                </span>
                              ) : product.isAvailableForRent ? (
                                <span className="text-lg font-bold text-teal">
                                  ₹{(product.rentPrice || 0).toLocaleString("en-IN")}<span className="text-[10px] font-medium text-text-muted">/month</span>
                                </span>
                              ) : null}
                            </div>
                          </div>

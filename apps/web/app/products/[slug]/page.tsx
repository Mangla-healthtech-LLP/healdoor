...
import { CheckCircle2, ArrowRight, IndianRupee, Phone } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      product.seo?.description || `Rent or Buy ${product.name} from HealDoor.`,
    alternates: {
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const formattedFaqs = product.faqs
    ?.map((faq) => {
  ...
  return (
    <>
      ...
      <main className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {Array.isArray(product.category) ? (
                      product.category.map(
                        (
                          cat: { name?: string; slug?: string } | string,
                          idx: number,
                        ) => {
                          const text =
                            typeof cat === 'string'
                          ...
                          return (
                            <span
                              key={idx}
                              className="inline-block px-4 py-1.5 bg-teal-light text-teal text-sm font-bold rounded-full w-fit uppercase tracking-wider"
                            >
                              {text.replace('-', ' ')}
                            </span>
                          )
                    ...
                    {product.rentPrice && product.isAvailableForRent && (
                      <div className="flex flex-col">
                        ...
                        <span className="text-2xl font-bold text-teal flex items-center">
                          ...
                          {product.rentPrice.toLocaleString('en-IN')}
                ...
                <div className="mb-4 space-y-4">
                  <div className="flex flex-row gap-2 sm:gap-3">
                    <Link
                      ...
                      className="flex-1 flex justify-center items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-3 sm:py-3.5 bg-teal hover:bg-teal-dark text-white rounded-xl font-bold text-sm sm:text-base transition-colors shadow-md shadow-teal/20"
                    >
                      Inquire
                  ...
                  <p className="text-sm text-text-muted text-center flex items-center justify-center gap-2 pt-2">
                  ...
                  {product.features && product.features.length > 0 && (
                    <div>
                      ...
                      {product.features.length > 3 && (
                        <a
                          href="#details"
                          className="text-teal hover:text-teal-dark font-medium mt-2 inline-block text-sm"
                        >
        ...
        <section
          id="details"
          className="py-6 lg:py-10 bg-section-alt-bg border-t border-border/50"
        >
              ...
              <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-border/50">
                ...
                <div className="grid sm:grid-cols-2 gap-x-8 gap-y-4">
                  {product.features.map((feature, idx) => (
                    <div
                      key={idx}
                      className="flex gap-3 py-2 border-b border-border/50 last:border-0 sm:last:border-b sm:[&:nth-last-child(2)]:border-0"
                    >
                      <CheckCircle2 className="w-5 h-5 text-teal shrink-0 mt-0.5" />
